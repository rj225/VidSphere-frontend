      ...
      {showPlaylistModal && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-30 backdrop-blur-sm flex items-center justify-center">
          <div className="bg-gray-800 p-4 w-4/12 h-3/6 overflow-auto rounded shadow-lg text-white relative">
            ...
            <h3 className="text-3xl text-cyan-200 text-center p-4 mb-4">Manage Playlists</h3>
            {playlists.map((playlist) => (
              <div key={playlist._id} className="flex items-center w-full pl-2 mb-2">
                <span className="w-2/4 text-xl">{FirstCapital(playlist.name)}</span>
                {playlist.videos.includes(selectedVideo._id) ? (
                  <button className=" flex items-center cursor-default w-2/4 mb-2 mx-4 rounded">
                    < IoCheckmarkCircleSharp className="text-xl shadow-inner shadow-green-700 rounded-full text-green-400 text-opacity-55 mr-1" /><span className="text-sm text-slate-500 italic mr-3">Added!</span>
                  ...
                  >
                    <IoAddCircleSharp className="hover:text-cyan-600 text-2xl mr-2 text-cyan-500 cursor-pointer" onClick={() =>
                      handleAddToPlaylist(playlist._id, selectedVideo._id)
                    }/>
                    <span className="text-base text-slate-100">Add to playlist</span>
                  </button>
                )}
              </div>
            ))}
